...
class CameraObserverService:
    ...
    def capture_frame(self, *, camera_index: int = 0) -> dict[str, object]:
        out_path = self.workspace.path_for_new_file(
            f"desk_observation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
        )
        commands = [
            ["rpicam-still", "-n", "-o", str(out_path), "-t", "1000"],
            ["libcamera-still", "-n", "-o", str(out_path), "-t", "1000"],
        ...
        cv2_result = self._capture_with_cv2(out_path, camera_index)
    ...
    def observe_once(self, *, camera_index: int = 0) -> dict[str, object]:
        capture = self.capture_frame(camera_index=camera_index)
    ...
    def _capture_with_cv2(self, out_path: Path, camera_index: int) -> dict[str, object]:
        try:
            import cv2
        except Exception:
